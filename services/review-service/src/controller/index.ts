...
import { updateRating } from "../queue";

const calculateRating = async (entityType: string, entityId: string) => {
  const reviews = await Review.find({ entityId, entityType });
...
const addReview = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const newRating = await calculateRating(
      validateData.entityType,
      validateData.entityId
    );
    await updateRating(
      validateData.entityId,
      validateData.entityType,
    ...
    if (error instanceof ZodError) {
      return res
        .status(400)
        .json(new ApiResponse(400, error.errors, "Validation Error!"));
    }
    return res
      .status(500)
...
const editReview = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (error instanceof ZodError) {
      return res
        .status(400)
        .json(new ApiResponse(500, error.errors, "Validation Error!"));
    }
    return res
      .status(500)
      .json(new ApiResponse(500, {}, "Internal server error!"));
...
const deleteReview = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Store entity details before deletion
    ...
    // Recalculate rating
    const newRating = await calculateRating(entityType, entityId);

    // Update entity rating
    await updateRating(entityId, entityType, newRating);
...
const getRestaurantReviews = asyncHandler(
    ...
    try {
      ...
      return res
        ...
        .json(
          new ApiResponse(
            200,
            { reviews, totalReviews, page: pageNum, limit: limitNum },
...
const getDeliveryAgentReviews = asyncHandler(
  async (req: Request, res: Response) => {
